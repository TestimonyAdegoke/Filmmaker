...
export default function AdminStudio({initialProjects,initialTracks,email}:{initialProjects:Project[];initialTracks:Track[];email:string}){
 ...
 async function createTrack(e:React.FormEvent<HTMLFormElement>){
   ...
   try{
     ...
     const payload={title,slug:slugify(title),genre:String(fd.get('genre')),duration:String(fd.get('duration')||''),audio_url:audio,artwork_url:art,featured:true,published:true,sort_order:tracks.length+1};
     const {data,error}=await supabase.from('tracks').insert(payload).select().single(); if(error)throw error;
     setTracks(t=>[...t,data as Track]); form.reset(); setStatus('Track added successfully.'); router.refresh();
 ...
 return <main className="admin-shell"><div className="admin-wrap">
   ...
   <div className="admin-grid">
    <section className="admin-panel"><h2>Add a project</h2><form className="admin-form" onSubmit={createProject}>
      ...
      <label>Roles<input name="roles" placeholder="Director · Composer · Score Engineer" required/></label><label>Short description<textarea name="description" rows={3} required/></label><label>Story / case study<textarea name="story" rows={5}/></label>
      <div className="admin-row"><label>Poster / artwork<input name="poster" type="file" accept="image/*"/></label><label>Hero video<input name="video" type="file" accept="video/*"/></label></div><div className="admin-row"><label>Poster URL (optional)<input name="poster_url" type="url" placeholder="https://…"/></label><label>Streaming video URL<input name="video_url" type="url" placeholder="Mux / Cloudinary / CDN URL"/></label></div><label>Trailer URL<input name="trailer_url" type="url" placeholder="YouTube / Vimeo / external trailer"/></label><div className="admin-row"><label>Accent colour<input name="accent" type="color" defaultValue="#7f5b4d"/></label><label>Custom slug<input name="slug" placeholder="auto-generated if empty"/></label></div>
      <div className="admin-row"><label><span>Featured</span><input name="featured" type="checkbox" defaultChecked/></label><label><span>Published</span><input name="published" type="checkbox" defaultChecked/></label></div><button className="admin-submit" disabled={busy}>{busy?'Uploading…':'Publish project ↗'}</button>
    ...
    <section className="admin-panel"><h2>Add a score / track</h2><form className="admin-form" onSubmit={createTrack}>
      <label>Track title<input name="title" required/></label><div className="admin-row"><label>Genre / mood<input name="genre" placeholder="ORCHESTRAL / AMBIENT" required/></label><label>Duration<input name="duration" placeholder="04:18"/></label></div><label>Audio file<input name="audio" type="file" accept="audio/*" required/></label><label>Artwork<input name="artwork" type="file" accept="image/*"/></label><button className="admin-submit" disabled={busy}>{busy?'Uploading…':'Add track ↗'}</button>
    </form><div className="admin-items">{tracks.map(t=><div className="admin-item" key={t.id}><div><strong>{t.title}</strong><small>{t.genre} · {t.duration||'—'}</small></div><button className="admin-button" onClick={()=>remove('tracks',t.id)}>Delete</button></div>)}</div></section>
   </div>
 </div></main>
}
